...
class StepController extends Controller
{
	...
	$index = 1;
     ...
     '//LIMIT '.($index-1)*$par_page.', '.($index)*$par_page
     );
     
      $query->setFirstResult(($index-1)*$par_page);
      $query->setMaxResults(($index)*$par_page);
      $list_step = $query->getResult();
      foreach($repo_step->findBy(array('ship'=>$ship)) as $step)
	$count++;
      
      $last_page = round($count / $par_page)+1;
