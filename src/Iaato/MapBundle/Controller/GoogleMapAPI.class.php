   ...
   protected $coordCoef = 0.01;
    ...
    public function init() 
    {
        ...
        if ($this->displayDirectionFields==true) {
            $this->content .= "\t\t".'html += \'<div style="clear:both;height:20px;"></div>\';'."\n";
            $this->content .= "\t\t".'id_name = \'marker_\'+gmarkers.length;'."\n";
            $this->content .= "\t\t".'html += \'<input type="text" id="\'+id_name+\'"/>\';'."\n";
            $this->content .= "\t\t".'from = lat+","+lng;'."\n";
            $this->content .= "\t\t".'idpanel = \''.$this->googleMapDirectionId.'\';'."\n";
            $this->content .= "\t\t".'html += \'<br /><input type="button" onClick="addDirection(from,document.getElementById(\\\'\'+id_name+\'\\\').value,idpanel);" value="Arrivée"/>\';'."\n";
            $this->content .= "\t\t".'html += \'<input type="button" onClick="addDirection(document.getElementById(\\\'\'+id_name+\'\\\').value,from,idpanel);" value="Départ"/>\';'."\n";
        }
        ...
        $this->content .= "\t".'function addDirection(from,to,idpanel) {'."\n";
        $this->content .= "\t\t".'directionsPanel = document.getElementById(idpanel);'."\n";
        $this->content .= "\t\t".'if (directions!=null) { directions.clear(); }'."\n";
        $this->content .= "\t\t".'directions = new GDirections(map, directionsPanel);'."\n";
        $this->content .= "\t\t".'directions.load("from: "+from+" to: "+to,"locale: '.$this->lang.'");'."\n";
        $this->content .= "\t\t".'map.closeInfoWindow();'."\n";
        $this->content .= "\t".'}'."\n";
        ...
        $this->content .= "\t\t".'for (var i=0; i<gmarkers.length; i++) {'."\n";
        $this->content .= "\t\t\t".'if (gmarkers[i].mycategory == category) {'."\n";
        $this->content .= "\t\t\t\t".'if (gmarkers[i].isHidden()) gmarkers[i].show();'."\n";
        $this->content .= "\t\t\t\t".'else gmarkers[i].hide();'."\n";
        $this->content .= "\t\t\t".'}'."\n";
        $this->content .= "\t\t".'}'."\n";
        $this->content .= "\t\t".'map.closeInfoWindow();'."\n";
        $this->content .= "\t".'}'."\n";
        ...
        $this->content .= "\t\t".'var oKML = new GGeoXml(file);'."\n";
        $this->content .= "\t\t".'map.addOverlay(oKML);'."\n";
        $this->content .= "\t".'}'."\n";
        
        // JS public function add a the wikipedia layer
        $this->content .= "\t".'function addLayerWikipedia() {'."\n";
        $this->content .= "\t\t".'layer_wikipedia = new GLayer("org.wikipedia.fr");'."\n";
        $this->content .= "\t\t".'map.addOverlay(layer_wikipedia);'."\n";
        $this->content .= "\t".'}'."\n";
        $this->content .= "\t".'function removeLayerWikipedia() {'."\n";
        $this->content .= "\t\t".'map.removeOverlay(layer_wikipedia);'."\n";
        $this->content .= "\t".'}'."\n";
        
        // JS public function add a the panoramio  layer
        $this->content .= "\t".'function addLayerPanoramio() {'."\n";
        $this->content .= "\t\t".'layer_panoramio = new GLayer("com.panoramio.all");'."\n";
        $this->content .= "\t\t".'map.addOverlay(layer_panoramio);'."\n";
        $this->content .= "\t".'}'."\n";
        $this->content .= "\t".'function removeLayerPanoramio() {'."\n";
        $this->content .= "\t\t".'map.removeOverlay(layer_panoramio);'."\n";
        $this->content .= "\t".'}'."\n";

        // JS public function add the traffic info
        $this->content .= "\t".'function addTrafficInfo() {'."\n";
        $this->content .= "\t\t".'var trafficOptions = {incidents:true};'."\n";
        $this->content .= "\t\t".'trafficInfo = new GTrafficOverlay(trafficOptions);'."\n";
        $this->content .= "\t\t".'map.addOverlay(trafficInfo);'."\n";
        $this->content .= "\t".'}'."\n";
        $this->content .= "\t".'function removeTrafficInfo() {'."\n";
        $this->content .= "\t\t".'map.removeOverlay(trafficInfo);'."\n";
        $this->content .= "\t".'}'."\n";
